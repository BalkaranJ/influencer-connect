...
// delete Campaign by the brand rep-------works
...
// Create Campaign details-----works

const createCampaignDetails =  function(db, userObject) {
  const querryString =
  (`
...
// set status to 'removed' by the brand manager --------works
const setUpdateStatus =  function(db, userObj) {
  const querryString =
  (`
  UPDATE tasks
  SET status = 'removed'
  ...
  return db.query(querryString, [userObj.taskId, userObj.userId])
};

// Update some info by brand manager
const updateTaskbyManager =  function(db, userObject) {
  const querryString =
  (`
  UPDATE tasks
  SET status = $1, start_date = $2, description = $3
    FROM campaigns,  campaign_details, brands, brand_managers, users
    WHERE campaign_id = campaigns.id AND campaign_detail_id = campaign_details.id AND campaign_details.brand_id = brands.id AND brand_managers.brand_id = brand_managers.id AND user_id = users.id
  AND tasks.id = $4 AND user_id = $5;
  `)

  return db.query(querryString, [userObject.status, userObject.start_date, userObject.description, userObject.task.id, userObject.user.id])
};
